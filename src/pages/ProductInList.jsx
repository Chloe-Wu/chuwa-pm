...
function ProductInList({ product }) {
  ...
  const user_token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjoiNjU4MTdkMjBiMTkyNmE5ZTVjMzQ3ZTUwIiwiYWRtaW4iOnRydWV9LCJpYXQiOjE3MDUzMTc3MDEsImV4cCI6MTcwNTQwNDEwMX0.r5G9SMSkzdA4d7n-9DhEPucDs5NKa1fudgqAazYeBL4";
  ...
  const userIncreaseProduct = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/api/user_increase_product/${productID}`, {},
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${user_token}`,
  ...
  const userDecreaseProduct = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3000/api/user_decrease_product/${productID}`, {},
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${user_token}`,
  ...
  const removeProduct = async () => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/user_remove/${productID}`,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="frame">
      ...
      <div className="buttons">
        {quantity > 0 ? (
          ...
          <Button className="button1" colorScheme="teal" onClick={addProduct}>
            Add to Cart
          </Button>
        )}
        {isAdmin && (
